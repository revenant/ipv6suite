...
Operatorset &op) {
 if( mf.getType() == MembershipFunction::CRISP )
  { return op.not2(compute_smeq( ((FuzzySingleton &) mf).getValue())); }
 ...
 if( mf.getType() == MembershipFunction::INNER &&
  ...
  for(int i=0; i<omf.length; i++){
   mu1 = omf.conc[i]->degree();
   mu2 = op.not2(compute_smeq(omf.conc[i]->param(0)));
   minmu = (mu1<mu2 ? mu1 : mu2);
   if( degree<minmu ) degree = minmu;
  }
 }
...
Operatorset &op) {
 if( mf.getType() == MembershipFunction::CRISP )
  { return op.not2(compute_greq( ((FuzzySingleton &) mf).getValue())); }
 ...
 if( mf.getType() == MembershipFunction::INNER &&
  ...
  for(int i=0; i<omf.length; i++){
   mu1 = omf.conc[i]->degree();
   mu2 = op.not2(compute_greq(omf.conc[i]->param(0)));
   minmu = (mu1<mu2 ? mu1 : mu2);
   if( degree<minmu ) degree = minmu;
  }
 }
...
Operatorset &op) {
 if( mf.getType() == MembershipFunction::CRISP )
  { return op.not2(compute_eq( ((FuzzySingleton &) mf).getValue())); }
 double mu1,mu2,minmu,degree=0;
 if( mf.getType() == MembershipFunction::INNER &&
     ((OutputMembershipFunction &) mf).isDiscrete() ){
  OutputMembershipFunction &omf = (OutputMembershipFunction &) mf;
  for(int i=0; i<omf.length; i++){
   mu1 = omf.conc[i]->degree();
   mu2 = op.not2(compute_eq(omf.conc[i]->param(0)));
   minmu = (mu1<mu2 ? mu1 : mu2);
   if( degree<minmu ) degree = minmu;
  }
 }
