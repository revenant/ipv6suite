                                            ...
                                            const char *nameFilter)
{
    ...
    IntVector outVec;
    const Values& scalars = scalarMgr->getValues();
    RunRef lastRunRef;
    bool lastRunMatched = false;
    for (Values::const_iterator i=scalars.begin(); i!=scalars.end(); i++)
    {
        const Datum& d = *i;
...
bool DataSorter::sameGroupFileRunScalar(const Datum& d1, const Datum& d2)
{
    return d1.runRef==d2.runRef && d1.scalarNameRef==d2.scalarNameRef;
}

bool DataSorter::sameGroupModuleScalar(const Datum& d1, const Datum& d2)
{
    return d1.moduleNameRef==d2.moduleNameRef && d1.scalarNameRef==d2.scalarNameRef;
}

bool DataSorter::sameGroupFileRunModule(const Datum& d1, const Datum& d2)
{
...
bool DataSorter::lessByModuleRef(int id1, int id2)
{
    ...
    const Datum& d1 = tmpScalarMgr->getValue(id1);
    const Datum& d2 = tmpScalarMgr->getValue(id2);
...
IntVector DataSorter::getModuleAndNamePairs(const IntVector& idlist, int maxcount)
{
    ...
    for (int ii = 0; ii < (int)idlist.size(); ii++)
    {
        ...
        {
            const Datum& vd = scalarMgr->getValue(*i);
