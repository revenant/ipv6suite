...
class DataSorter
{
  private:
    ...
    typedef bool (*GroupingFunc)(const Datum&, const Datum&);
    typedef bool (*CompareFunc)(int id1, int id2);

  private:
    // comparison functions for doGrouping() and sortAndAlign()
    static bool sameGroupFileRunScalar(const Datum& d1, const Datum& d2);
    static bool sameGroupModuleScalar(const Datum& d1, const Datum& d2);
    static bool sameGroupFileRunModule(const Datum& d1, const Datum& d2);
