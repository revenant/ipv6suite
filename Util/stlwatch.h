...
class cMapWatcher : public cVectorWatcherBase
{
  protected:
    ...
    std::string classname;
  public:
    cMapWatcher(const char *name, std::map<_K,_V,_C>& var) : cVectorWatcherBase(name), m(var) {
        itPos=-1;
        classname = std::string("std::map<")+opp_typename(typeid(_K))+","+opp_typename(typeid(_V))+">";
    }
    const char *className() const {return classname.c_str();}
