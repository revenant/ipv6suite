...
void InterfaceTable::initialize(int stage)
{
    ...
    InterfaceTableParser parser(this);
    if (*filename && parser.readInterfaceTableFromFile(filename)==-1)
...
bool InterfaceTable::localDeliver(const IPAddress& dest)
{
...
bool InterfaceTable::multicastLocalDeliver(const IPAddress& dest)
{
...
RoutingEntry *InterfaceTable::selectBestMatchingRoute(const IPAddress& dest)
{
...
int InterfaceTable::outputPortNo(const IPAddress& dest)
{
...
IPAddress InterfaceTable::nextGatewayAddress(const IPAddress& dest)
{
...
MulticastRoutes InterfaceTable::multicastRoutesFor(const IPAddress& dest)
{
...
int InterfaceTable::numRoutingEntries()
{
    return routes.size()+multicastRoutes.size();
}

RoutingEntry *InterfaceTable::routingEntry(int k)
{
...
RoutingEntry *InterfaceTable::findRoutingEntry(const IPAddress& target,
...
void InterfaceTable::addRoutingEntry(RoutingEntry *entry)
{
...
bool InterfaceTable::deleteRoutingEntry(RoutingEntry *entry)
{
...
bool InterfaceTable::routingEntryMatches(RoutingEntry *entry,
